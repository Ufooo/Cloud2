...
import {
    ...
    store,
    update,
} from '@/actions/Nip/Composer/Http/Controllers/ComposerController';
...
const editUpdateUrl = computed(() => {
    ...
    return update.url({ site: props.site, credential: editingCredential.value.id });
});
...
async function handleDelete(credential: Credential) {
    ...
    router.delete(
        destroy.url({ site: props.site, credential: credential.id }),
        {
            preserveScroll: true,
        }
    );
...
<template>
    <Head :title="`Composer - ${site.domain}`" />
